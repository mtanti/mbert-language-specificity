...
import json
import itertools
from typing import Mapping, Optional, Tuple, Sequence, Dict, List, Callable, Union, Iterator
import torch
...
from mufins.experiments.lang_ent_max_cls.hyperparameters import Hyperparameters
from mufins.experiments.lang_ent_max_cls.training_process import ModelTrainingProcess
...
class Net(nn.Module):
    ...
    def forward(
        ...
        :param label_token_indexes: Integer tensor of token indexes to be
            used for predicting the labels.
        :param label_token_mask: Boolean tensor of token masks to be
            used for predicting the labels.
        :param lang_token_indexes: Integer tensor of token indexes to be
            used for predicting the language.
        :param lang_token_mask: Boolean tensor of token masks to be
            used for predicting the language.
...
#########################################
class LangEntMaxClsModel(ModelAdversarial):
    '''
    The model class.
    '''
    ...
    def __init__(
        ...
        self.trainable_module: Optional[str] = None
    ...
    def get_label_predictions(
        ...
        :return: A triple consisting of the following:
            ...
            - A list with an item for each text consisting of a list of the log_e probabilities of
                each label.
            - A list of the names of the most likely label for each text.
            - A list of the names of the true label for each text.
        '''
        if dset_label.spec != self.label_spec:
            raise ValueError('Dataset is incompatible with model.')
