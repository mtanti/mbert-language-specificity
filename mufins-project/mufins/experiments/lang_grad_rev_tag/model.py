...
from mufins.experiments.lang_grad_rev_tag.hyperparameters import Hyperparameters
...
from mufins.experiments.lang_grad_rev_tag.training_process import ModelTrainingProcess
...
class Net(nn.Module):
    ...
    def forward(
        ...
        if lang_token_indexes is not None and lang_token_mask is not None:
            ...
            content_word_vecs = self.dropout_layer(content_word_vecs)
            if self.training:
                content_word_vecs = grad_rev_layer(content_word_vecs, self.grl_lambda)
...
#########################################
class LangGradRevTagModel(ModelStandard):
    '''
    The model class.
    '''
    ...
    def _set_gradients(
        ...
        if self.get_curr_opt_index() == 0: # Main
            ...
            for lang_subbatch in get_subbatches(lang_batch, batch_size):
                ...
                (label_logits, lang_logits) = self.__batch_logits(None, lang_tensors)
                assert lang_tensors is not None
                assert lang_logits is not None
