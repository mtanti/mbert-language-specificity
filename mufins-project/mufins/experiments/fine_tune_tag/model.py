'''
Neural network backend model for the experiment.
'''
...
from mufins.experiments.fine_tune_tag.hyperparameters import Hyperparameters
from mufins.common.model.model_standard import ModelStandard
from mufins.experiments.fine_tune_tag.training_process import ModelTrainingProcess
...
#########################################
class FineTuneTagModel(ModelStandard):
    '''
    The model class.
    '''
    ...
    def get_lang_predictions(
        ...
        for batch in dset.get_batches(batch_size):
            ...
            for row_idx in range(predicted_langs.shape[0]):
                ...
                true = list()
                for col_idx in range(batch['num_tokens'][row_idx] - 2):
                    if 'label_mask' not in batch or batch['label_mask'][row_idx, col_idx].tolist():
                        prob.append(
                            logprobs[row_idx, col_idx].tolist()
                        )
    ...
    def _set_gradients(
        ...
        '''
        if self.get_curr_opt_index() == 0:
            (label_batch,) = batch
            num_labels = label_batch['label_mask'].sum()

            for label_subbatch in get_subbatches(label_batch, batch_size):
                (label_tensors, _) = self.__batch_prepare_inputs(
                    label_subbatch, None
                )
                (label_logits, _) = self.__batch_logits(label_tensors, None)
                ...
                error = torch.sum(errors*label_tensors['label_mask'])/num_labels

                error.backward()

        elif self.get_curr_opt_index() == 1:
            (lang_batch,) = batch
