'''
Run experiment with ID: lang_grad_rev_cls
'''
...
from mufins.dataprocs.xnli.preprocess import DATASET_VERSION as XNLI_DATASET_VERSION
...
from mufins.experiments.lang_grad_rev_cls.hyperparameters import (
    LangGradRevClsParameterSpace
)
from mufins.experiments.lang_grad_rev_cls.model import LangGradRevClsModel
from mufins.experiments.lang_grad_rev_cls.evaluate import (
...
def _output_results(
    ...
    dset_label_test: Dataset[XNLIDataRow],
    ...
    '''
    Calculate the label evaluation results of a single model and save outputs to file.
    ...
    if not hyperparameter_search_mode:
        results_file.append(
            [
                ...
                '{:.10f}'.format(label_label_score_acc),
...
#########################################
def lang_grad_rev_cls_experiment(
    label_src_path: str,
    lang_src_path: str,
    dst_path: str,
    device_name: str,
    parameter_space: LangGradRevClsParameterSpace,
    hyperparameter_search_mode: bool,
    batch_size: int,
    seed: Optional[int],
    verbose: bool = True,
    ...
    '''
    Run experiment lang_grad_rev_cls.

    :param label_src_path: The path to the directory with the preprocessed XNLI dataset.
    :param lang_src_path: The path to the directory with the preprocessed UDPOS dataset.
    :param dst_path: The path to the directory to contain the experiment1
        results.
    ...
    try:
        ...
        with Timer() as overall_timer:
            log.log_message('')
            log.log_message('------------------------------------------------')
            log.log_message('Running lang_grad_rev_cls experiment.')
            log.log_message('')
            ...
            with open(os.path.join(label_src_path, 'data_spec.json'), 'r', encoding='utf-8') as f:
                label_spec = XNLIDataSpec.from_json(f.read().strip())
            ...
            version_file.init()
            if XNLI_DATASET_VERSION != version_file.read():
            ...
            dset_label_train = DatasetFile[XNLIDataRow](
                os.path.join(label_src_path, 'dataset_train.hdf'),
                label_spec,
            )
            dset_label_val = DatasetFile[XNLIDataRow](
                os.path.join(label_src_path, 'dataset_val.hdf'),
                label_spec,
            )
            if not hyperparameter_search_mode:
                dset_label_test = DatasetFile[XNLIDataRow](
